...
app.post('/login', async (req, res) => {
  ...
  db.query('SELECT * FROM Users WHERE email = ?', [email], async (err, result) => {
    ...
    if (result.length > 0) {
      ...
      const user_model={user_id: user.user_id, emailId: user.email, usertype: user.usertype}
...
app.post('/add-product', (req, res) => {
  ...
  db.query('INSERT INTO Varieties (name, reducedPrice, quantity, about, discount) VALUES (?, ?, ?, ?, ?)', 
...
// Route to handle adding a new order and order items
app.post('/add-order-items', async (req, res) => {
  try {
    const { user_id, order_items } = req.body;

    // Determine the order ID
    let orderId = 1; // Default order ID if no previous orders exist

    // Retrieve the highest existing order ID from the database
    const highestOrderId = await getHighestOrderId();
    if (highestOrderId) {
      orderId = highestOrderId + 1; // Increment the highest order ID by one
    }

    // Insert the order items into the database
    for (const orderItem of order_items) {
      const { product_id, quantity, product_total_cost } = orderItem;
      await db.query(
        'INSERT INTO order_items (order_id, product_id, user_id, quantity, product_total_cost) VALUES (?, ?, ?, ?, ?)',
        [orderId, product_id, user_id, quantity, product_total_cost]
      );
    }
...
// Function to retrieve the highest existing order ID from the database
const getHighestOrderId = () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT MAX(order_id) AS maxOrderId FROM order_items', (error, results) => {
      if (error) {
        console.error('Error retrieving highest order ID:', error);
        reject(error);
      } else {
...
// Define a route to fetch ordered items
app.post('/orders', (req, res) => {
  // Extract user ID from the request body
  const { user_id } = req.body;

  // Query the database to retrieve ordered items for the specified user
  db.query('SELECT * FROM order_items', (error, results) => {
    ...
    console.log(results)
    // Send the retrieved ordered items as a response
    res.status(200).json({ success: true, orderedItems: results });
  });
});

// Backend API Endpoint
app.post('/calculate-update-averages', async (req, res) => {
  try {
    // SQL query to calculate average ratings for each product
    const sql = `
    UPDATE varieties v
    JOIN (
      ...
      FROM reviews
      GROUP BY productId
    ) r ON v.id = r.productId
    SET v.rating = r.avgRating;
    
    `;
    
    // Execute the SQL query to update the varieties table
    db.query(sql, (error, results) => {
      if (error) {
        console.error('Error calculating and updating averages:', error);
        return res.status(500).json({ success: false, error: 'Internal Server Error' });
      }
      // Return success response
      return res.status(200).json({ success: true, message: 'Average ratings updated successfully' });
    });
  } catch (error) {
    console.error('Error calculating and updating averages:', error);
...
// Update quantity route
app.post('/update-quantity/:productId', async (req, res) => {
  const productId = req.params.productId;
  const newQuantity = req.body.quantity;
  
  try {
    // Update the quantity of the product in the varieties table
    const updateResult = await db.query('UPDATE varieties SET quantity = quantity - ? WHERE id = ?', [newQuantity, parseInt(productId)]);
